...
: "${PMS_APP_USERNAME:=pms_app@test}"
: "${PMS_MIGRATOR_USERNAME:=pms_migrator@test}"
...
app_sql() { MYSQL_PWD="$PMS_APP_PASSWORD" "${mysql_base[@]}" --user="$PMS_APP_USERNAME" --execute "$1"; }
migrator_sql() { MYSQL_PWD="$PMS_MIGRATOR_PASSWORD" "${mysql_base[@]}" --user="$PMS_MIGRATOR_USERNAME" --execute "$1"; }
root_sql() { MYSQL_PWD="$OCEANBASE_ROOT_PASSWORD" "${mysql_base[@]}" --user="${OCEANBASE_ROOT_USER:-root@test}" --execute "$1"; }
