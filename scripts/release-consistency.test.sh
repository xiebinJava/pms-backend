...
latest_migration="$({ find "$BACKEND_DIR/src/main/resources/db/migration" -maxdepth 1 -type f -name 'V*.sql' -exec basename {} \; 2>/dev/null || true; } \
  ...
  | sort -n \
  | tail -n 1)"
[[ "$latest_migration" == "41" ]] || fail "expected current migration baseline V1–V41, found V${latest_migration:-unknown}"
...
echo "Release consistency checks passed: migration=V1–V41, READMEs=current, feedback routes=documented"
