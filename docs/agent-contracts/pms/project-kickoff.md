...
读取能力绑定到当前已经注册的 `pms_project_list`、`pms_project_get`、`pms_task_list` 和 `pms_query`。写入能力绑定到：

`project.create`、`project.update`、`member.add`、`node.owner.update`、`node.schedule.update`、`task.create`、`task.assign`、`task.update`、`node.complete`。

尚未注册的命令不会被契约宣称为可执行能力。

`project.update` 是**部分更新**：只有显式传入的字段会改变（`name`、`description`、`priority`、`projectLevel`、`orgUnitId`、`startDate`、`endDate`），未提供的字段保留原值，显式传 `null` 可清空描述或日期。它不会改动项目成员、关注人和项目经理——需要维护成员请用 `member.add`（移出成员、归档、删除、回滚节点等命令当前不在契约授权范围内）。

## 执行规则

1. 先读取当前项目、节点、成员和任务状态。
2. 缺少项目经理、成员、组织单元、优先级、等级或关键日期时先询问，不猜测。
3. 批量写入统一生成一份预览，展示字段、警告、操作编号和刷新范围。
4. 只有用户在后续消息中明确确认同一份预览后才能执行。
