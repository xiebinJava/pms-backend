...
需要一次性完成“升级两次 + 完整预检”时，显式开启实时校验入口：

```bash
export PMS_OCEANBASE_VERIFY=true
./scripts/verify-oceanbase.sh
```

该入口不会删除数据库或表；第二次升级必须输出所有版本 checksum 校验通过且无待执行迁移。
...
```bash
./scripts/verify-backup.sh /var/backups/pms/brad_pms-<UTC时间>-v11.sql.gz
```
...
./scripts/restore-oceanbase.sh --allow-empty-target \
  /var/backups/pms/brad_pms-<UTC时间>-v11.sql.gz
./scripts/verify-enterprise-migration.sh
```
...
- 现有 `brad_pms` 的 V1–V21 被记录并可重复校验；第二次升级无待执行版本。历史演练记录中的 V1–V12 是当时的发布快照，不代表当前基线。
- 2026-09-05 本机目标库已使用已有 OceanBase CE 4.3.5 镜像内的 `obclient` 执行 V18–V21；V20 删除需求澄清基线的 `objective`、`deliverable` 列，V21 增加需求-任务关联表，V21 checksum、readiness=`migration=21` 和结构预检通过。该记录只覆盖本机目标库，正式环境仍需按本手册完成备份、升级和恢复演练。
- 备份压缩流和 SHA-256 校验通过，文件名带 `v11` 版本标识，元数据使用逐表 `COUNT(*)` 精确行数。
