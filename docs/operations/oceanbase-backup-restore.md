...
- 现有 `brad_pms` 的 V1–V18 被记录并可重复校验；第二次升级无待执行版本。历史演练记录中的 V1–V12 是当时的发布快照，不代表当前基线。
- 2026-09-05 方案设计节点上线前，本机目标库已使用已有 OceanBase CE 4.3.5 镜像内的 `obclient` 执行 V18；V18 checksum、readiness=`migration=18` 和结构预检通过。该记录只覆盖本机目标库，正式环境仍需按本手册完成备份、升级和恢复演练。
