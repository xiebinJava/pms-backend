...
3. 启动带新迁移的后端。Flyway 会按 V1–V55 顺序只执行缺失版本；V37 增加计划节点迭代计划及故事关联字段，V38/V39 更新计划节点文案，V40 增加知识沉淀与标准改进工作台，V41 增加方案评审版本字段，V42–V55 补充流程模板、研发工作项、软删除、可空项目上下文、自动项目成员引用及故事流程节点上下文。`spring.sql.init.mode` 已关闭，不会重复执行 `schema.sql`。
...
空库；后端启动时由 Flyway 按 V1–V55 执行缺失版本，不再单独执行 `schema.sql`。已有生产库
