...
| 检查项 | 当前状态 | 证据 |
| --- | --- | --- |
| MySQL `pms` | V1–V21 已执行并复核 | V1–V21 已在本机目标库完成；V20 删除需求澄清基线中重复的 `objective`、`deliverable` 列，V21 增加需求与任务关联表，并通过 readiness、关键表结构、外键和索引预检；正式环境仍需按发布账号执行并留存 checksum 证据 |
| 后端回归 | 已通过 | `mvn -q test`（新增需求-任务关联与进度统计覆盖）；权限、通知查询/提醒、项目等级、需求范围基线、方案设计评审决策、审计查询、脱敏、迁移和既有企业回归全部通过；健康检查代码基线已更新到 V21 |
| 前端回归 | 已通过 | `npm test`；通知中心、审计工作台、权限页面、项目等级、需求范围基线、需求-任务轻量联动、方案设计评审决策与既有模块测试通过 |
...
| 发布基线 | 代码与门禁已更新，生产发布未签字 | 后端与前端 `main`/`release` 已同步；跨仓库集成运行 `33705799079` 全部通过；两个仓库的 `v1.0.4` 正式标签已推送（`v1.0.3` 保留为历史基线），远程不使用 `master` |

## 仍需完成

| 优先级 | 缺口 | 影响 |
| --- | --- | --- |
| P1 | 生产目标环境的备份介质、RPO/RTO 和故障联系人仍需确认 | 本机目标库已完成 V1–V21 升级校验；生产环境仍需按企业 RPO/RTO 保存备份介质与联系人 |
| P1 | GitHub `main` 分支保护仍未启用，当前私有仓库套餐的保护 API 返回 403 | 跨仓库 CI 应覆盖 V1–V21 迁移、API 冒烟和浏览器验收；需升级 GitHub 套餐或将仓库公开后配置 PR 审查和必需检查，凭据轮换与镜像发布权限仍按部署组织策略执行 |
| P2 | Java 21 升级尚未规划 | 当前保持 Java 17；Spring Boot 3.5 依赖升级已完成，Java 21 留待后续兼容性窗口 |

## 2026-09-03 发布准备执行记录

- 后端已重建并以 mysql profile 重启；`GET /api/health/live` 与 `GET /api/health/ready` 均返回 200，ready body 为 `database=UP`、`migration=15`。健康检查基线已同步到 V15。
- 本机 MySQL 已完成 V1–V15 升级、企业结构校验和完整性预检；详细证据与生产边界见本次通知中心 Review。
- 当前后端 `mvn -q test`、前端 `npm test`、`npm run typecheck` 与 `npm run build` 均通过；OpenAPI、生产配置门禁、脚本语法和隐私扫描均通过。
- 后端独立 CI `33705799078` 已通过单元测试、OpenAPI、生产配置、密钥/隐私扫描、镜像构建、Trivy HIGH/CRITICAL 扫描和 SPDX SBOM；本次将 AWS SDK 所用 Netty 从 `4.1.132.Final` 提升到已修复的 `4.1.136.Final`。
...
- 当前代码的后端 readiness 期望迁移版本为 V21；本机 MySQL `pms` 已复核为 V21，ready 返回 `database=UP`、`migration=21`。
- V18 已新增方案包、三类评审和方案决策表，不删除或重建现有业务表；方案设计节点接口已完成本机启动和页面联调验收。
- V20 已删除 `project_node_baseline.objective` 和 `project_node_baseline.deliverable`；需求澄清节点只维护范围项和需求清单，历史两列数据随迁移删除。
- 项目等级按参考体系落为常规 C/0、重要 B/1、关键 A/2、战略 S/3；新建项目默认常规 C，项目等级与优先级（交付紧急程度）独立。
- 需求基线工作台已接入项目详情；确认前要求至少一条纳入范围项、至少一条需求且全部需求完成确认，确认后可重新打开再修订。
- 需求清单支持从已确认需求行创建任务并自动带入需求编号，需求行按关联任务回显完成进度；未确认需求不能创建关联任务，任务状态变化不会自动确认需求。
...
4. 本地开发 MySQL 只允许作为当前开发实例特例，生产环境必须使用非空强密码。
...
- T2 MySQL 演练：本机 MySQL 8.4 上以 `pms_migrator` 完成 V1–V10 两次幂等校验，以 `pms_app` 完成只读企业预检；v10 zstd 备份、SHA-256、35 张表精确行数元数据、隔离空库恢复及关键表行数 `21/9/2/7/21` 比对通过。篡改 checksum 和指向现有 `pms` 的恢复均按预期拒绝；详见 [`drill-records/2026-09-02-production-readiness.md`](drill-records/2026-09-02-production-readiness.md)。
...
- T3 数据库：本机 MySQL 8.4 的 `pms` 使用 `pms_migrator` 完成 V1–V15 升级复核；重复执行输出 `No pending migrations. All migration checksums verified.`，未删除数据库或业务表。
- T4 只读校验：`bash scripts/verify-enterprise-migration.sh`（复用 MySQL 镜像内 `mysql`）通过，企业迁移校验和完整性预检全部通过；组织历史表、索引、外键、单根组织、归属关系和项目关联均无异常。
- T5 Review：后端 `mvn -q test` 194 个用例（0 失败、0 错误、1 跳过）；前端 `pnpm test` 112 项、`pnpm typecheck`、`pnpm build`、OpenAPI 校验和脚本语法检查通过。初次工具镜像拉取因网络超时，已改用本地已有 MySQL 镜像，不影响迁移结果。
...
| Task 1：MySQL 运行账号隔离 | 已完成 | 配置测试通过；Compose 解析通过；本地 MySQL 已创建 `pms_app` / `pms_migrator`；应用账号建表被拒绝、迁移账号 DDL 通过；探针表已清理。 |
| Task 2：MySQL 备份、恢复与版本升级 | 已完成本机演练 | 2026-09-01 重新执行 V1–V10 checksum/幂等、精确行数逻辑备份、SHA-256、隔离空库恢复和关键表比对；备份版本标记与全表元数据问题已修复；详见最新演练记录。 |
...
| Task 7：集成 CI/CD、安全扫描与浏览器冒烟 | 已完成 | Trivy Action 已固定为有效的 `v0.36.0`；前端 `package.json` 与 CI/集成工作流统一使用 `pnpm@9.15.9`；前端 `a8977d4` 的 `pnpm test`（112 项）、类型检查和构建均通过；后端跨仓库运行 `33596130518` 已通过 MySQL、迁移幂等、API 冒烟、浏览器代理、项目夹具及桌面/移动端 Playwright 验收；浏览器用例改为应用内导航，避免把内存会话误测成刷新令牌流程 |
| Task 8：生产演练与开源交付 | 本机演练完成，生产注入待目标企业 | MySQL V1–V10、备份/恢复、应用健康检查、邮箱登录和本机 Playwright 验收已记录；生产 JWT、SMTP、HTTPS、CORS、对象存储与监控告警需由部署企业注入并复验。 |
...
- 本次变更已在本地目标 MySQL 执行 V7，并重新跑 preflight/verify 全部通过；当前用户数据均已具备可归一化邮箱。V8–V10 随后补充任务附件、站内通知和通知节点标识；其他部署环境必须按完整 V1–V10 顺序执行并保存校验结果。

### Task 1：发布基线

- 后端 `v1.0.0` 标签：`7cc2820`，已合并 `main`；标签之后仅追加文档收口提交。
...
- 前端 CI `33351102816` 已通过（测试、类型检查、构建、Nginx 镜像 Trivy 和 SPDX SBOM）。后端升级后 CI `33352990972` 已通过单元测试、OpenAPI、密钥扫描、后端镜像 Trivy HIGH/CRITICAL 扫描和 SPDX SBOM。
- 最新集成运行 `33596130518` 已通过。后端工作流使用只读 `PMS_FRONT_REPO_READ_TOKEN` 检出前端 `a8977d4`，并在干净 MySQL 上创建浏览器验收项目夹具；旧的凭据预检失败记录保留在历史演练文档中。
...
- `MysqlRuntimeConfigurationTest`、`NotificationConfigurationValidatorTest`、`ContainerHardeningTest`：通过；Compose 配置解析：通过；私钥材料扫描：无命中。
...
### Task 4：MySQL 备份恢复与故障演练

- 已通过现有 `pms-mysql` 容器内的 `mysql` 完成只读 `SELECT 1` 连通性确认。
- `enterprise-preflight.sh` 和 `verify-enterprise-migration.sh` 已在同一 MySQL 容器内通过；使用官方镜像内 `/u01/mysql/bin/mysqldump` 完成 gzip 备份、SHA-256 校验、精确行数元数据和隔离空库恢复，源库/恢复库关键表行数一致。
- 详细证据和安全边界见最新 [`drill-records/2026-09-01-release-acceptance.md`](drill-records/2026-09-01-release-acceptance.md) 以及历史记录。恢复仅写入明确空目标库，不对现有 `pms` 执行覆盖式恢复。
