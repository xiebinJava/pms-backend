...
> 说明：本文下方的 V21、V15 等内容是对应日期的历史验收快照；当前发布基线已推进到 V41，当前安装与升级请以 [README](../../README.md)、[企业升级手册](enterprise-upgrade-runbook.md) 和 `scripts/release-consistency.test.sh` 为准。

## 当前基线

| 检查项 | 当前状态 | 证据 |
| --- | --- | --- |
| OceanBase `brad_pms` | V1–V21 已执行并复核 | V1–V21 已按 `pms_migrator` 流程在本机目标库完成；V20 删除需求澄清基线中重复的 `objective`、`deliverable` 列，V21 增加需求与任务关联表，并通过 readiness、关键表结构、外键和索引预检；正式环境仍需按发布账号执行并留存 checksum 证据 |
...
- 当前代码的后端 readiness 期望迁移版本为 V21；本机真实 OceanBase `brad_pms` 已复核为 V21，ready 返回 `database=UP`、`migration=21`。
