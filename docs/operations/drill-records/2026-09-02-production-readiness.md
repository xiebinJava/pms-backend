...
本记录只保存本机目标 MySQL 实例和本地应用验收结果，不代表任意企业生产环境已经完成认证。密码、JWT、令牌和 SMTP 内容均通过运行时环境注入，未写入记录。
...
| 本机开发配置误用生产校验 | 按预期拒绝 | `.env.mysql.local` 保持开发设置；未将本地开发值伪装成生产签字 |
...
| 使用 `--allow-empty-target` 恢复到已有 `pms` | 按预期拒绝，退出码 2；未执行写入 |
...
```bash
# 迁移两次、结构验收和只读预检
PMS_MYSQL_VERIFY=true ./scripts/verify-enterprise-migration.sh

# 备份与完整性校验
PMS_BACKUP_DIR=/secure/backup/path ./scripts/backup-mysql.sh
./scripts/verify-backup.sh /secure/backup/path/<backup-file>.sql.zst

# 仅恢复到明确新建的空库；禁止指向现有生产库
./scripts/restore-mysql.sh --allow-empty-target /secure/backup/path/<backup-file>.sql.zst
