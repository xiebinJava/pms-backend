...
需求澄清与范围基线节点在独立的节点工作台中维护目标、交付结果、纳入/不纳入范围项和需求清单。需求清单用于把已经澄清的内容拆成可追踪的需求条目，供后续方案、开发和验收引用；它不是任务看板，也不替代任务执行记录。每个需求条目包含稳定编号、名称、类型、优先级、验收标准和确认状态。节点只有在范围基线已确认、至少有一条纳入范围项、至少有一条需求且全部需求已确认时，才允许完成。

需求范围基线与节点一一对应：草稿可以由有节点编辑能力的责任人维护，确认后锁定；如需修订，先重新打开，再保存草稿并重新确认。基线带版本号，保存使用乐观锁，版本冲突返回 409，不会用旧页面覆盖他人的修改。重新打开会把现有需求条目恢复为待评审；保存时新增或内容发生变化的需求也会恢复为待评审，必须重新确认。已确认内容不会因普通页面刷新丢失，子项由服务端事务整体替换保存，接口仍受项目可读和节点生命周期约束。

方案设计、评审与决策节点在需求范围基线确认后维护一个方案包，不维护多个候选方案或任意评分。方案包包含方案版本、产品方案、技术方案、方案摘要、范围覆盖和上线前提；需求范围基线只作为只读上游输入展示。方案包提交后必须完成三类固定评审：业务/产品评审、技术评审、测试/发布评审；每类评审各一条，状态为待完成或已通过。三类评审全部通过后，责任人填写决策结果（通过、条件通过或退回修改）和决策依据；选择条件通过时还必须填写通过条件。决策确认后锁定方案包、评审和决策内容，若需修改必须先重新打开，方案包内容发生变化会使评审重新回到待完成。该节点只有在方案包已提交、三类评审全部通过且决策已确认时才能完成；保存使用乐观锁，版本冲突返回 409，所有写入仍受项目权限和节点生命周期约束。
...
- 表结构按 V1–V18 版本化迁移顺序执行：V1–V7 完成基础企业模型与邮箱身份，V8 增加任务附件，V9 增加站内通知，V10 增加通知节点标识，V11 增加组织变更历史和导入失败状态字段，V12 增加反馈工单及追加式处理历史，V13 增加项目图片访问边界，V14 增加审计项目上下文、原因、结果、IP 和 User-Agent 字段及查询索引，V15 增加通知提醒去重键及唯一索引，V16 增加项目等级字段 `project.project_level`，编码为 0 常规（C）、1 重要（B）、2 关键（A）、3 战略（S），默认值为常规（0）；V17 增加需求澄清节点的范围基线、范围项和需求清单表；V18 增加方案包、三类评审和方案决策表。
