...
本次 review 覆盖当前企业版发布基线：迁移版本与文档合同、OpenAPI、CI 门禁、MySQL
...
3. Actuator 邮件健康检查改为显式开关：未配置 SMTP 时默认不参与 readiness；只有目标环境
   ...
   被误判为 DOWN”。
4. 补充本次 MySQL 演练记录、发布清单和基础设施状态，保留外部企业环境的待办边界。
...
`05f241f`，MySQL、迁移幂等、API 冒烟、登录代理、桌面端和移动端 Playwright 均通过。
...
- 注入强 JWT、MySQL、SMTP、HTTPS 和受限 CORS，并确认默认密码已更换、旧令牌失效。
...
本机代码、文档、门禁、MySQL 恢复链路和远程集成验收已达到“可审计的发布候选”状态，
