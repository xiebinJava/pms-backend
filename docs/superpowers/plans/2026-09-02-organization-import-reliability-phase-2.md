...
**Tech Stack:** Spring Boot, MyBatis-Plus, Flyway, MySQL (MySQL mode), Apache Commons CSV, Apache POI, Vue 3, Ant Design Vue, Vitest/Vue Test Utils.

**Spec:** `docs/business-specification.md` and `docs/superpowers/specs/2026-08-27-enterprise-organization-access-design.md`

## Global Constraints

- MySQL is the only supported runtime database; do not add in-process database behavior.
...
2. **Schema and persistence** ✅
   - Add an MySQL-compatible Flyway migration for append-only `sys_org_unit_history` and import-job idempotency/status metadata.
...
5. **Verification and delivery** ✅
   - Run unit/integration tests, Flyway validation and MySQL migration verification, API smoke checks, and frontend typecheck/build/tests.
...
- MySQL 8.4 `pms`: V1–V11 applied with `pms_migrator`; a second run verified every checksum and reported no pending migrations.
- `bash scripts/verify-enterprise-migration.sh` passed using the locally available MySQL image and `mysql`; no database or business table was dropped.
