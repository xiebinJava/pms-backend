...
- Test: `../pms-front/src/plugins/http/http.test.mjs`
...
- [x] **Step 6: Commit**
  ...
  ```bash
  git add src/main/java src/main/resources/application.yml src/test/java ../pms-front/src/plugins/http ../pms-front/src/plugins/http/http.test.mjs
...
**Files:**
- Modify: `Dockerfile`
- Modify: `../pms-front/Dockerfile`
- Modify: `docker-compose.example.yml`
- Modify: `../pms-front/nginx.conf`
...
- [x] **Step 5: 验证容器**
  ...
  docker build -t pms-front:local ../pms-front
  ```
...
- Create: `../pms-front/tests/e2e/auth-and-project.spec.ts`
- Modify: `../pms-front/package.json`
...
- [x] **Step 6: Commit（代码待复核后提交）**
  ...
  git add ../pms-front/.github ../pms-front/tests ../pms-front/package.json
...
- [x] **Step 6: Commit（文档待复核后提交）**
  ...
  ```bash
  git add docs README.md ../pms-front/README.md
