...
- Final verification: frontend `corepack pnpm test` — 274/274 passing; `corepack pnpm build` — passing. Backend targeted Maven regression — 113 tests across 16 suites, 0 failures/errors. Visual QA covered desktop/mobile layouts, node preview, keyboard reorder, and pointer drag-and-drop; no browser console errors. `git diff --check` is clean in both worktrees.
