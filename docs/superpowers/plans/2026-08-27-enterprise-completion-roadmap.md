...
- Modify: `../pms-front/src/views/admin/roles/index.vue`
- Modify: `../pms-front/src/views/admin/users/index.vue`
...
- Test: `../pms-front/src/router/admin-guard.test.mjs`
...
- Modify: `../pms-front/src/views/admin/org/OrgCanvas.vue`
- Modify: `../pms-front/src/views/admin/org/index.vue`
- Modify: `../pms-front/src/views/admin/users/index.vue`
...
- Test: `../pms-front/src/views/admin/org/org-canvas.test.mjs`

**Deliverable:** 组织画布全屏可拖拽、缩放和居中，所有父子节点连线正确；节点展示组织类型和负责人；新增、移动、停用组织和调整员工主归属后，人员页即时显示最新主归属。

- [x] **Step 1:** 为树结构连线、孤儿节点、循环父级、负责人独立于员工归属和停用组织迁移补充测试。
- [x] **Step 2:** 后端增加树校验、批量移动和停用前置校验，保留负责人历史和员工任职历史；V11 追加不可变组织变更历史。
- [x] **Step 3:** 前端画布改为固定可视区域 + 无限舞台，使用 pointer 拖拽和缩放，不因窗口变小挤压节点；连线按节点实际坐标绘制。
- [x] **Step 4:** 人员列表按最新有效主归属查询，显示兼职/项目归属，不把组织负责人关系混入主归属字段。
- [x] **Step 5:** 运行组织和人员测试并进行浏览器视觉 review，组织历史查询接口已补齐。
...
- Modify: `../pms-front/src/views/admin/import/index.vue`
- Test: `src/test/java/com/brad/pms/service/EnterpriseImportServiceTest.java`
- Test: `../pms-front/src/views/admin/import/import.test.mjs`

**Deliverable:** 提供组织模板和员工模板，支持 CSV/XLSX 上传、预览、错误行下载、英文名解析、负责人解析、主归属/兼职归属校验和全有全无提交。

- [x] **Step 1:** 为重复英文名、缺失父组织、循环组织、负责人不存在、主归属为空和重复导入编写失败测试。
- [x] **Step 2:** 实现模板下载、大小/行数限制、编码识别、字段级错误和预览摘要；预览不写业务表。
- [x] **Step 3:** 提交时使用单事务和幂等批次号，失败自动回滚，保留导入批次及错误审计；V11 补充失败原因和时间。
- [x] **Step 4:** 前端增加“上传 → 预览 → 确认 → 结果”四步状态，错误行支持服务端下载，成功后刷新组织与人员列表。
- [x] **Step 5:** 运行导入测试和 `pnpm typecheck`，完成大文件、重复提交和部分失败路径 Review。
...
- Modify: `../pms-front/src/views/admin/audit/index.vue`
- Modify: `docs/operations/enterprise-upgrade-runbook.md`
