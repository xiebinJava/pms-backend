...
- 默认宽度：560px。
- 可拖拽范围：360px–720px。
- 默认收起，不打开时 DSH 只显示普通 AI 对话。
- 支持收起、恢复、最大化和在新页面打开。
- 打开后直接嵌入真实 PMS 页面，保留 PMS 顶栏、左侧导航、项目管理和配置管理等完整页面能力；摘要视图仅作为加载失败或快速概览时的降级方案。
- 第一版通过 allowlist 校验后的 PMS URL 直接加载；生产环境可切换到同源受控工作区路由，避免在浏览器中暴露长期凭据。
- 收起或关闭只隐藏业务工作区，不解除当前 DSH 会话的 PMS 关联；新会话默认不绑定 PMS。
...
PMS 工作区内部默认加载真实 PMS 页面，例如：

```text
/projects
```

开发环境可以暂时映射到 `http://127.0.0.1:5174/projects`；生产环境使用配置的受控 PMS 工作区 URL 或 DSH 同源路由，由 DSH 服务端完成一次性嵌入认证。不能让浏览器直接携带长期 PMS 凭证访问跨域地址。

PMS 被 iframe 加载时仍保留完整页面壳；只有显式传入 `embed=1` 才允许使用紧凑壳。PMS 响应必须配置允许的 `frame-ancestors`，并拒绝未知 DSH 来源。嵌入页面与 DSH 之间的 `postMessage` 必须校验 `origin`、`sessionId` 和消息版本。
...
- 项目切换后 iframe / 原生摘要同步。
- 收起、关闭、重新打开后会话仍保持 PMS 标签和最近一次 locator。
- PMS 页面加载失败时的降级提示。
- 写操作完成后当前页面刷新。
- 刷新 DSH 页面后会话按用户恢复。
