...
> 项目域的读取、创建、责任写入和生命周期规则已在 2026-09-03 的
> `project-permission-consolidation-design.md` 中收口；本文保留组织/身份中心的历史设计，项目域如有冲突以新规范为准。
...
- 项目列表的历史基线曾按组织范围或项目成员过滤；现行项目域规则以
  `project-permission-consolidation-design.md` 为准：持有 `project:read` 的员工可查看全部未删除项目及其项目内容。
