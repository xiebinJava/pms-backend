...
- **账号模型：** 中文姓名用于人员展示，英文名用于账号登录。展示固定为 `中文名（English.Name）`；英文登录名不区分大小写。
...
| 字段 | 规则 |
| --- | --- |
| `name_zh` | 中文姓名，必填；由旧 `nickname` 回填。 |
| `username` | 英文名的展示写法，例如 `Brad.Xie`，必填。 |
| `username_normalized` | `username` 的 `Locale.ROOT` 小写值，例如 `brad.xie`；唯一索引为跨 H2/MySQL/OceanBase 的大小写无关唯一性保障。 |
...
1. 登录请求先以 `Locale.ROOT` 将英文名转为小写，再按 `username_normalized` 精确查询；密码使用 BCrypt 校验。
...
- 左侧组织树筛选，右侧人员表格展示 `中文名（英文名）`、主归属、岗位、角色、账号状态。
- 支持按中文名、英文名、岗位、组织和角色查询；管理抽屉集中编辑账号状态、主归属、兼职、岗位、角色和数据范围。
...
- 支持 Excel 与 CSV 模板下载、上传、预览、字段校验和结果下载。组织导入以组织编码和父组织编码建树；员工导入包含中文名、英文名、邮箱/手机号、主组织编码、岗位、角色、直属上级英文名。
...
1. 迁移前备份生产库并运行只读预检：检查用户名归一化冲突、孤儿项目、缺失项目成员用户与组织编码冲突。
2. 使用版本化迁移创建新表、索引和 `project.org_unit_id`；创建“公司总部”和“待分配人员”组织。
3. 旧 `nickname` 迁移到 `name_zh`，旧 `username` 迁移到展示英文名和归一化英文名。若归一化冲突，迁移停止并报告冲突账号，不自动改名。
...
- 以 `Brad.Xie`、`brad.xie`、`BRAD.XIE` 均可登录同一账号，且不能创建大小写不同的重复英文名。
- 所有人员展示为 `中文名（英文名）`；项目成员、负责人、评论和审计日志无旧 `nickname` 单独展示。
