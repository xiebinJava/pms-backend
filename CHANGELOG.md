...
- Production-readiness phase 1 local verification: production configuration gate tests passed; database V1–V10 idempotency, backup checksum, isolated restore and refusal boundaries passed; the smoke test now sends email-first login payloads while retaining username compatibility; current backend test report is 190 tests (0 failures, 0 errors, 1 skipped), frontend is 112/112, and local desktop/mobile Playwright acceptance passed. Enterprise production credentials and infrastructure remain deployment-owned inputs.

## Unreleased

- Runtime, local Compose, Helm, and tests use MySQL 8 only; Flyway runs with the backend, and tests use Testcontainers.
- Use MySQL-safe `TEXT`/`MEDIUMTEXT` for long JSON and solution fields so Flyway can apply V1–V41 on MySQL 8.4 `utf8mb4`.
- Map `orgUnitId` to `org_unit_id` so organization updates no longer query a missing `orgunitid` column.
- Point first-time installers to the public `pms-distribution` repository and use the published GitHub clone URLs.
- Release-closure snapshot (2026-09-01): `mvn -q test` 187 cases passed (0 failures, 0 errors, 1 skipped); live readiness returned `migration=10`; V1–V10 backup/recovery and key-table row-count comparison passed.
...
- Add a contributor MySQL 8 start path (`docker-compose.mysql.yml` + Flyway) against database `pms`.
...
- Add a Helm chart for backend + frontend. It does not install MySQL; ServiceMonitor and Ingress stay off by default.

## 1.0.0

- Single-tenant self-hosted backend on MySQL 8.
