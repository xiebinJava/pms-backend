...
public interface PermissionMapper extends BaseMapper<PermissionDO> {
    ...
    @Select("SELECT DISTINCT ur.role_id FROM sys_user_role ur JOIN sys_role_permission rp ON rp.role_id = ur.role_id JOIN sys_permission p ON p.id = rp.permission_id WHERE ur.user_id = #{userId} AND p.code = #{permissionCode} AND ur.status = 'ACTIVE' AND (ur.start_at IS NULL OR ur.start_at <= CURRENT_TIMESTAMP) AND (ur.end_at IS NULL OR ur.end_at > CURRENT_TIMESTAMP)")
