...
public class RollbackNodeCommand implements PmsCommand {
    ...
    public CommandName name() {
        return CommandName.NODE_ROLLBACK;
    }
    ...
    public CommandPreview preview(CommandPreviewRequest request) {
        ...
        ProjectNodeDO node = permissionService.requireRollbackableNode(projectId, nodeId);

        Map<String, Object> change = new LinkedHashMap<>();
        change.put("entity", "node");
        change.put("action", "rollback");
        change.put("projectId", projectId);
        change.put("nodeId", nodeId);
        change.put("nodeName", node.getName());
        change.put("reason", reason);
        change.put("fromStatus", NodeStatus.COMPLETED.getCode());
        change.put("toStatus", NodeStatus.IN_PROGRESS.getCode());
        ...
        return new CommandPreview(null, name(), Instant.now().plusSeconds(600), request.contextVersion(),
                List.of("回退会把目标节点之后的节点恢复为未开始，并重新计算项目进度"),
    ...
    public CommandResult execute(AiOperationDO operation) {
        ...
        permissionService.requireRollbackableNode(projectId, nodeId);
        List<ProjectNodeDTO> nodes = nodeService.rollback(projectId, nodeId, reason);
        return new CommandResult(operation.getId(), "SUCCEEDED", "节点已回退", Map.of(
                "projectId", projectId,
                "nodeId", nodeId,
                "reason", reason,
    ...
    private void assertFresh(AiOperationDO operation, Integer projectVersion, Integer nodeVersion) {
        try {
            ...
            throw BusinessException.error("节点回退预览版本信息无效");
        }
    }
    ...
    private Map<String, Object> readArguments(String json) {
        try {
            ...
            throw BusinessException.error("节点回退参数无效");
        }
    }
}
